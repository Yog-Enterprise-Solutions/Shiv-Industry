...
def execute(filters=None):
	...
	message = get_message()    
	# frappe.log_error("report data","\n\n\n".join(list(map(str,data))))
	item_price_map = {}
	for ind, record in enumerate(data):
		# if records['supplier_name']:
		# 	supplier = supplier
		item = record['item_code']
		if item_price_map.get(item):
			if data[item_price_map[item][0]]['price'] > record['price']:
				item_price_map[item] = [ind,True]
			item_price_map[item][1]= True
		else:
			item_price_map[item] = [ind,False]
	
	# frappe.log_error("ITEM MAP" , item_price_map)
	for v in item_price_map.values():
		if v[1]:
			data[v[0]]['flag'] = True
	# frappe.log_error("DATA flag" , "\n\n\n".join(list(map(str,data))))
